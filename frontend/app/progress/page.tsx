...
export default function ProgressPage() {
  ...
  const scoreColor = (pct: number) =>
    pct >= 70 ? "#e8844a" : pct >= 40 ? "#f5c842" : "#ef4444";
      ...
      <nav className="relative z-20 flex items-center justify-between px-5 sm:px-8 py-4 border-b border-white/[0.06]">
        <Link href="/" className="flex items-center gap-3">
          <svg width="16" height="20" viewBox="0 0 20 24" fill="none">
            ...
            <defs>
              <linearGradient id="fg5" x1="10" y1="0" x2="10" y2="24" gradientUnits="userSpaceOnUse">
          ...
          <div className="flex flex-col gap-8">
            ...
            {data.total_quizzes === 0 && (
              <div className="bg-[#141414] border border-white/[0.06] rounded-sm p-10 text-center">
                ...
                <Link href="/quiz"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-[#e8844a] text-[#0c0c0c] font-bold text-xs tracking-[0.15em] uppercase rounded-sm hover:bg-[#f09558] transition-colors">
            ...
            {data.total_quizzes > 0 && (
              <>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/[0.06]">
                  {[
                    { label: "Quizzes Taken", value: data.total_quizzes, unit: "" },
                    { label: "Average Score", value: data.average_score, unit: "%" },
                    { label: "Best Score", value: data.best_score, unit: "%" },
                    { label: "Current Level", value: data.current_difficulty, unit: "", isText: true },
                  ].map((s) => (
                    <div key={s.label} className="bg-[#0c0c0c] p-6 flex flex-col gap-2">
                      ...
                      <p className="text-3xl font-black tracking-tight"
                        style={{color: s.isText ? (difficultyColor[s.value as string] || "#e8844a") : "#f0ede8"}}>
                        {s.value}{s.unit}
                      </p>
                    </div>
                  ))}
                ...
                {data.topics.length > 0 && (
                  <div>
                    <p className="text-[10px] tracking-[0.2em] uppercase text-[#3a3530] mb-4">Topic Mastery</p>
                        ...
                        <div key={t.topic} className="bg-[#141414] border border-white/[0.06] rounded-sm p-4">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-[10px] text-[#3a3530] tracking-wider">{t.attempts} attempt{t.attempts !== 1 ? "s" : ""}</span>
                            </div>
                            <span className="text-sm font-bold" style={{color: scoreColor(t.average)}}>
                              {t.average}%
                            </span>
                          </div>
                          {/* progress bar */}
                        ...
                        <div key={a.id} className="bg-[#141414] border border-white/[0.06] rounded-sm px-4 py-3 flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <span className="shrink-0 text-[10px] font-bold uppercase px-2 py-0.5 rounded-sm border"
                              style={{
                                color: difficultyColor[a.difficulty] || "#e8844a",
                                borderColor: `${difficultyColor[a.difficulty]}40` || "rgba(232,132,74,0.4)",
                              }}>
                              {a.difficulty}
                            </span>
                          </div>
                          <div className="flex items-center gap-4 shrink-0">
                            ...
                            <span className="text-sm font-bold w-12 text-right"
                              style={{color: scoreColor(a.percentage)}}>
                              {a.percentage}%
                            </span>
                            <span className="text-[10px] text-[#2a2520] hidden sm:block">
