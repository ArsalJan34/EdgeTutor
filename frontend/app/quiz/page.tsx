  ...
  const generateQuiz = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/quiz/generate?topic=${encodeURIComponent(topic)}&num_questions=${numQuestions}&difficulty=${difficulty}`
      );
      ...
      <nav className="relative z-20 flex items-center justify-between px-5 sm:px-8 py-4 border-b border-white/[0.06] shrink-0">
        <Link href="/" className="flex items-center gap-3">
          <svg width="16" height="20" viewBox="0 0 20 24" fill="none">
            ...
            <defs>
              <linearGradient id="fg4" x1="10" y1="0" x2="10" y2="24" gradientUnits="userSpaceOnUse">
            ...
            <div className="flex flex-col gap-5">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. recursion, linked lists, sorting algorithms..."
                  ...
                  {[3, 5, 10].map((n) => (
                    <button key={n} onClick={() => setNumQuestions(n)}
                      className={`px-5 py-2 rounded-sm text-sm font-bold transition-colors duration-200 border ${
                        numQuestions === n
                          ? "bg-[#e8844a] border-[#e8844a] text-[#0c0c0c]"
                          : "border-white/[0.08] text-[#6b6560] hover:text-[#f0ede8] hover:border-white/20"
                      }`}>
                      {n}
                    </button>
                  ))}
              ...
              {error && (
                <div className="px-4 py-3 rounded-sm text-sm border border-red-500/20 bg-red-500/5 text-red-400">{error}</div>
              )}
        ...
        {(stage === "quiz" || stage === "submitting") && (
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] tracking-[0.2em] uppercase mb-1" style={{color: difficultyColor[difficulty]}}>
                  {difficulty} · Quiz
                </p>
              ...
              <div className="text-right">
                ...
                <div className="mt-1.5 w-32 h-1 bg-[#1c1c1c] rounded-full overflow-hidden">
                  <div className="h-full bg-[#e8844a] transition-all duration-300 rounded-full"
                    ...
                    {optionLabels.map((label) => (
                      <button key={label} onClick={() => setAnswers((prev) => ({ ...prev, [qi]: label }))}
                        className={`flex items-center gap-3 px-4 py-3 rounded-sm text-sm text-left transition-all duration-150 border ${
                          answers[qi] === label
                            ? "border-[#e8844a] bg-[#e8844a]/10 text-[#f0ede8]"
                            : "border-white/[0.06] text-[#a39d97] hover:border-white/20 hover:text-[#f0ede8]"
                        }`}>
            ...
            <button onClick={submitQuiz} disabled={stage === "submitting"}
              className="w-full py-4 rounded-sm font-bold text-xs tracking-[0.15em] uppercase transition-all duration-200 bg-[#e8844a] hover:bg-[#f09558] text-[#0c0c0c] disabled:opacity-50">
              {stage === "submitting" ? (
        ...
        {stage === "results" && scoreData && (
          <div className="flex flex-col gap-6">
            <div className="bg-[#141414] border border-white/[0.06] rounded-sm p-7 sm:p-8 text-center">
              ...
              <p className="text-[#3a3530] text-xs mt-1 tracking-wider">
              ...
              <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 border border-white/[0.06] rounded-sm">
                ...
                <span className="text-xs font-bold uppercase"
                  style={{color: difficultyColor[scoreData.next_difficulty as Difficulty] || "#e8844a"}}>
                  {scoreData.next_difficulty}
                </span>
              </div>
            </div>
              ...
              {scoreData.results.map((r, i) => (
                <div key={i} className={`bg-[#141414] border rounded-sm p-5 ${r.correct ? "border-[#e8844a]/20" : "border-red-500/15"}`}>
                  <div className="flex items-start gap-3 mb-3">
                    <span className={`shrink-0 mt-0.5 text-xs font-bold ${r.correct ? "text-[#e8844a]" : "text-red-400"}`}>
                      {r.correct ? "✓" : "✕"}
                    </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 mb-3">
                    {optionLabels.map((label) => (
                      <div key={label} className={`flex items-center gap-2 px-3 py-2 rounded-sm text-xs border ${
                        label === r.correct_answer
                          ? "border-[#e8844a]/30 bg-[#e8844a]/5 text-[#e8844a]"
                          : label === r.selected && !r.correct
                  ...
                  <p className="text-[#6b6560] text-xs leading-relaxed border-t border-white/[0.05] pt-3">
                    <span className="text-[#3a3530] tracking-wider uppercase text-[10px] mr-1">Explanation:</span>
                    {r.explanation}
                  </p>
                </div>
              ))}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={reset}
                className="flex-1 py-4 rounded-sm font-bold text-xs tracking-[0.15em] uppercase bg-[#e8844a] hover:bg-[#f09558] text-[#0c0c0c] transition-colors">
                Try Another Quiz
              </button>
              <Link href="/progress"
                className="flex-1 py-4 rounded-sm font-bold text-xs tracking-[0.15em] uppercase border border-white/[0.08] text-[#6b6560] hover:text-[#f0ede8] hover:border-white/20 transition-colors text-center">
                View Progress
              </Link>
            </div>
          </div>
