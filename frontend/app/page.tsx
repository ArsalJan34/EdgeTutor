    ...
    <main className="min-h-screen bg-[#0c0c0c] text-[#f0ede8] flex flex-col relative overflow-hidden">
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 sm:px-10 py-5 border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/upload" className="hidden sm:flex text-xs font-medium tracking-widest uppercase text-[#a39d97] hover:text-[#f0ede8] transition-colors px-4 py-2 border border-white/[0.08] hover:border-white/20 rounded-sm">Upload</Link>
          <Link href="/progress" className="hidden sm:flex text-xs font-medium tracking-widest uppercase text-[#a39d97] hover:text-[#f0ede8] transition-colors px-4 py-2 border border-white/[0.08] hover:border-white/20 rounded-sm">Progress</Link>
          <Link href="/chat" className="flex text-xs font-medium tracking-widest uppercase bg-[#e8844a] hover:bg-[#f09558] text-[#0c0c0c] transition-colors px-4 py-2 rounded-sm">Chat</Link>
        </div>
      </nav>

      {/* HERO */}
      <section className="relative z-10 flex flex-col items-start flex-1 px-6 sm:px-10 lg:px-16 pt-16 pb-12 max-w-6xl w-full mx-auto">
        <div className="mb-6">
          <span className="inline-block text-[10px] font-medium tracking-[0.25em] uppercase text-[#e8844a] border border-[#e8844a]/30 px-3 py-1.5 rounded-sm">AI-Powered Learning</span>
        </div>
        <h1 className="text-[clamp(2.8rem,8vw,6.5rem)] font-black leading-[0.92] tracking-[-0.03em] text-[#f0ede8] mb-6 uppercase max-w-4xl">
          Study Smarter.<br /><span style={{color:"#e8844a"}}>Ask Anything.</span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <Link href="/upload" className="flex items-center justify-center gap-2 px-8 py-4 bg-[#e8844a] hover:bg-[#f09558] text-[#0c0c0c] font-bold text-sm tracking-[0.12em] uppercase transition-all rounded-sm w-full sm:w-auto">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
            </svg>
            Upload PDF
          </Link>
          <Link href="/chat" className="flex items-center justify-center gap-2 px-8 py-4 border border-white/[0.12] hover:border-[#e8844a]/40 text-[#a39d97] hover:text-[#f0ede8] font-medium text-sm tracking-[0.12em] uppercase transition-all rounded-sm w-full sm:w-auto">
            Ask AI Tutor
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-px bg-white/[0.06]">
          {[
            ...
            <Link key={f.num} href={f.href}
              className="group bg-[#0c0c0c] hover:bg-[#141414] transition-colors duration-300 p-6 sm:p-7 flex flex-col gap-4">
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-medium tracking-[0.2em] text-[#e8844a]" style={{fontFamily:"var(--font-geist-mono)"}}>{f.num}</span>
                <span className="text-[10px] tracking-[0.2em] uppercase text-[#3a3530]">{f.label}</span>
              </div>
              <div>
                <h3 className="text-[#f0ede8] font-bold text-base mb-1.5 group-hover:text-white transition-colors">{f.title}</h3>
                <p className="text-[#6b6560] text-xs leading-relaxed">{f.desc}</p>
              </div>
              <div className="mt-auto flex items-center gap-1.5 text-[10px] text-[#3a3530] group-hover:text-[#e8844a] transition-colors">
                <span className="tracking-[0.1em] uppercase font-medium">Open</span>
